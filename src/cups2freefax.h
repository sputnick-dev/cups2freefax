/*
   cups2freefax.h -- CUPS Backend Header File
   based on cups-pdf.h -- CUPS Backend Header File (version 2.4.8, 2008-06-22)
...
#define CPCONFIG "/etc/cups/cups2freefax.conf"
...
/  options listed below can be set via cups2freefax.conf at runtime		*/

#define CPVERSION "v2.4.8"
...
static void _set_defaults() {
  snprintf(conf.anondirname,BUFSIZE,"%s","/var/spool/cups2freefax/ANONYMOUS");
  ...
  snprintf(conf.postprocessing,BUFSIZE,"%s","/var/lib/cups2freefax/cups2freefax.bash");
  snprintf(conf.out,BUFSIZE,"%s","/var/spool/cups2freefax/${USER}");
  snprintf(conf.spool,BUFSIZE,"%s","/var/spool/cups2freefax/SPOOL");
...
static int _assign_value(char *key, char *value) {
  ...
  if (!strcmp("AnonDirName",key))
    ...
    tmp=atoi(value);
    conf.label=(tmp)?(short)1:(short)0;
  }
